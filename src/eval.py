...
class Evaluator(teras.training.event.Listener):
    ...
    def __init__(self, parser, rel_map, gold_file, logger=None, **kwargs):
        ...
        self._logger = logger if logger is not None else teras.utils.logging.getLogger("teras")
            ...
            records.append(
                {
                    ...
                    "pred_rels": [rel_map.lookup(rel if rel > 0 else 0) for rel in p_rels[1:]],
                    "true_heads": t_heads[1:],
                    "true_rels": [rel_map.lookup(rel if rel > 0 else 0) for rel in t_rels[1:]],
                    "head_errors": np.where(p_heads[1:] != t_heads[1:])[0].astype(np.int32) + 1,
                    "rel_errors": np.where(p_rels[1:] != t_rels[1:])[0].astype(np.int32) + 1,
                }
            )
    ...
    def report(self, show_details=False):
        sentences, heads, rels = zip(
            *[(r["sentence"], r["pred_heads"], r["pred_rels"]) for r in self._parsed["records"]]
        )
        ...
        if result["code"] == 0:
            ...
            message = "[evaluation] ERROR({}): {}".format(result["code"], result["raw"].rstrip())
...
EVAL_SCRIPT = os.path.join(os.path.abspath(os.path.dirname(__file__)), "common", "eval.pl")


def exec_eval(parsed_file, gold_file, show_details=False):
    command = ["/usr/bin/perl", EVAL_SCRIPT, "-g", gold_file, "-s", parsed_file]
    if not show_details:
        command.append("-q")
    ...
    if p.returncode == 0:
        lines = output.split("\n", 2)[:2]
        result["LAS"], result["UAS"] = [float(line.rsplit(" ", 2)[-2]) for line in lines]
    return result
